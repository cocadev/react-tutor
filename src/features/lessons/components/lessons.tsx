...
const Lessons: React.FC<Props> = ({ data }) => {
  ...
  return data?.length ? (
    <Box>
      <Masonry className={classes.masonry} breakpointCols={breakpointColumnsObj}>
        {data
          ...
          .sort(sortByDates())
          .map(lesson => (
            <Fade key={lesson._id} top>
              <Cards.Link to={ROUTES.dynamic.lessons(lesson._id)}>
                <Cards.Lesson lesson={lesson} />
              </Cards.Link>
            </Fade>
          ))}
      </Masonry>
    ...
    <Info.Empty title="No lessons found..." />
  );
};
